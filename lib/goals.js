 ...
 *
 * 主人拍板（L1-L4 一批）：看板执行会话播种 goal（自主推进预算）、结算感知 goal
...
export const GOAL_ROUNDS_BY_LEVEL = {
    L0: undefined,
    L1: 2,
    L2: 3,
    L3: undefined,
...
export function goalCreateSpec(sessionId, objective, maxGoalRounds) {
 ...
 */
export function foldGoalFromRecords(records) {
    let folded = undefined;
    for (const r of records) {
        const ev = r?.event;
        if (ev === undefined || ev.type !== 'goal/change')
            continue;
        const data = ev.data;
        ...
        if (phase !== 'active' && phase !== 'paused' && phase !== 'blocked' && phase !== 'complete')
            continue;
        if (typeof data.goal.objective !== 'string' || data.goal.objective === '')
            continue;
        folded = {
            status: 'current',
            phase,
