...
export function apply(ctx) {
    ...
    try {
        tools.register({
            name: 'task_delegate',
            description: '把主任口头布置的工作立项为看板任务（可立即执行）。凡主任交代的多步骤、耗时、需要跟进或定时性的工作，' +
    ...
    try {
        tools.register({
            name: 'task_claim',
            description: '认领一个看板任务到当前会话执行（主任说"同意/开始/继续做"时使用）。认领即治理裁决：' +
    ...
    try {
        tools.register({
            name: 'task_approve',
            description: '批准等待审批的看板任务（主任专用）。任务执行被治理拦截（L2+ 待审批）后，主任说"同意/批准"时调用：' +
            ...
            execute: async (args, exec) => {
                ...
                const r = (await ledger.approve(pending.id, { by: '主任会话', via: '命令' }));
                if (r?.ok !== true)
