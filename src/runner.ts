...
export class TaskRunner {
  ...
  async inspect(sessionId: string, startedAt: number, opts: { goalSeeded?: boolean; deferTurnEnd?: boolean } = {}): Promise<ExecutionOutcome> {
    ...
    if (opts.goalSeeded === true || opts.deferTurnEnd === true) {
      ...
      if (opts.deferTurnEnd === true) return { outcome: 'pending' }
    }
