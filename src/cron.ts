...
/** 解析单字段表达式为命中值集合。 */
function parseField(raw: string, spec: FieldSpec, name: string): Set<number> {
  const values = new Set<number>()
  for (const part of raw.split(',')) {
    const piece = part.trim()
    if (piece === '') throw new CronError(`cron ${name} 字段存在空片段: "${raw}"`)
    // 步进：a/b 或 */b 或 a-b/b
    const stepSplit = piece.split('/')
    if (stepSplit.length > 2) throw new CronError(`cron ${name} 字段步进语法非法: "${piece}"`)
    const rangeText = stepSplit[0]!
    const step = stepSplit.length === 2 ? Number(stepSplit[1]) : 1
    if (!Number.isInteger(step) || step < 1) throw new CronError(`cron ${name} 字段步进必须为正整数: "${piece}"`)
    ...
    if (rangeText === '*') {
      ...
      if (bounds.length !== 2) throw new CronError(`cron ${name} 字段范围非法: "${piece}"`)
      lo = Number(bounds[0])
      hi = Number(bounds[1])
    } else {
      lo = Number(rangeText)
    ...
    if (!Number.isInteger(lo) || !Number.isInteger(hi)) throw new CronError(`cron ${name} 字段含非数字: "${piece}"`)
    // 周日 7 归一到 0
    if (name === 'dow') {
      if (lo === 7) lo = 0
      if (hi === 7) hi = 7 // 0-7 范围包含 0；单独 7 已归一
    }
    if (lo < spec.min || hi > spec.max || lo > hi) {
      throw new CronError(`cron ${name} 字段越界: "${piece}"（合法 ${spec.min}-${spec.max}）`)
    }
    for (let v = lo; v <= hi; v += step) values.add(name === 'dow' && v === 7 ? 0 : v)
  }
  if (values.size === 0) throw new CronError(`cron ${name} 字段为空`)
  return values
}
...
export function parseCron(expr: string): ParsedCron {
  ...
  if (parts.length !== 5) throw new CronError(`cron 表达式必须为 5 个字段（分 时 日 月 周）: "${expr}"`)
...
export function parseCached(expr: string): ParsedCron {
