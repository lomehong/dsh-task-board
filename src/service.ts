...
export class TaskBoardService {
  ...
  async refreshActivity(): Promise<void> {
    ...
    // 只取进行中（active）的自主目标——objective 截断 40 字，封顶 3 条保护主人注意力。
  ...
  async run(taskId: string, trigger: '手动' | '定时'): Promise<RunRecord> {
    ...
    // 3) 放行：投递分身会话（本地降级时 summary 带降级标注，供主人审阅）
  ...
  claim(taskId: string, sessionId: string, trigger: '手动' | '定时' = '手动'): RunRecord {
    ...
    // 注意：接管必须在 transact 内改 store——taskOf 返回的是解析副本，改副本不落盘。
    let sameSessionRunning = false
    transact((store) => {
      const live = store.tasks.find(x => x.id === taskId)
      if (live === undefined) return
      const running = live.runs.find(r => r.status === '运行中')
      if (running === undefined) return
      if (running.sessionId === sessionId) {
        sameSessionRunning = true
        return
      }
      running.status = '已取消'
      running.finishedAt = new Date().toISOString()
      running.summary = `执行现场由会话 ${sessionId} 接管（原会话 ${running.sessionId ?? '未知'}）`
      live.lastStatus = '已取消'
    })
    if (sameSessionRunning) {
      return this.recordRun(taskId, {
        id: genId('RUN'), startedAt: startedAtIso, status: '已阻断', trigger,
        summary: '该任务已由当前会话认领且正在执行——直接干活，完成后 task_report 上报',
      }, { keepColumn: true })
    }
    let verdict
    try {
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      // b1) 自动归档（P1，主人拍板）：已完成满 7 天 → 归档。看板永远只呈现
      //「当前要关心的活」；归档任务数据保留，客户端「含归档」开关可查、可恢复。
      // 只有「已完成」自动归档；已失败是需要主人注意的信号，永不自动归档。
      ...
      for (const task of store.tasks) {
        ...
        for (const run of running) {
          ...
          transact((s2) => {
            ...
            // 列语义（主人拍板的验收语义延伸）：取消 ≠ 失败——已取消=执行没走完，
            // 任务回「待办」待重做；只有真实失败才落「已失败」
            t.column = finish.status === '成功' ? '已完成' : finish.status === '已取消' ? '待办' : '已失败'
          })
  ...
  private recordRun(taskId: string, run: RunRecord, opts?: { failColumn?: boolean; keepColumn?: boolean }): RunRecord {
    transact((store: TaskBoardStore) => {
      ...
      if (t === undefined) return
      t.runs.push(run)
      if (t.runs.length > MAX_RUNS_PER_TASK) t.runs.shift()
...
/** 供路由层构造（每个宿主进程一个服务实例）。 */
export function createService(gateway: TypertGateway, options?: ServiceOptions): TaskBoardService {
