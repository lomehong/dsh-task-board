 ...
 * 均为已发生的客观事实，不是候选猜测）。写入后，主人在任何会话问
...
export interface TaskMemoryModule {
  addMemoryEntry?(entry: {
    ...
    verify?: { status: string; method?: string; at?: string }
  }): Promise<unknown> | unknown
}

let memoryGetter: (() => TaskMemoryModule | undefined) | undefined

/**
 * 注入 dsh-memory 的惰性获取器（插件 apply 时由 index.ts 接线）。
 * getter 返回 undefined 视为缺席 → 记忆沉淀显式降级。
 */
export function injectMemoryGetter(getter: () => TaskMemoryModule | undefined): void {
  memoryGetter = getter
}
